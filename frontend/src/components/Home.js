...
function Home() {
  ...
  useEffect(() => {
    const quoteInterval = setInterval(() => {
      ...
      // Delay to switch the quote after fade-out is complete
      setTimeout(() => {
        setCurrInd((prevIndex) => (prevIndex + 1) % length);
        setFadeState("fade-in"); // Start fade-in effect
      }, 1000); // Wait for fade-out to finish before updating the quote
    }, 5000);
  ...
  return (
    <div className="bg-yellow-600 h-screen">
      ...
      <section className="pt-[100px] text-center bg-yellow-600">
        ...
        <div className={fadeState + " py-20 quote-container w-100"}>
          <div>
      ...
      {/* Siblings like <footer> do not automatically inherit the background from the parent */}
      {/* FOOTER */}
      <footer className="py-2 bg-yellow-600">
